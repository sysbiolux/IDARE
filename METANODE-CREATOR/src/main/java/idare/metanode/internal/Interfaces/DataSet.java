...
import idare.metanode.internal.CSVReader.CSVWorkbook;
...
import idare.metanode.internal.DataSetReaders.POIReader.POIWorkBook;
...
//		TranslationToObject = new HashMap<>();
	 ...
	 */
	public boolean parseFile(File DataFile) throws WrongFormat,DuplicateIDException,InvalidFormatException,IOException{
		SourceFile = DataFile;		
		...
		{
			try{
				PrintFDebugger.Debugging(props, "Testing validity of a dataset for these properties");
			...
			{
				PrintFDebugger.Debugging(props, "Data not valid for property because: " + e.getMessage());
	...
	/**
	 * Read the datafile, first we try to read it as an xls file otherwise we assume a tab separated csv file.
	 * @param DataFile
	 * @throws IOException
	 * @throws WrongFormat
	 ...
	 */
	protected void readDataFile(File DataFile) throws IOException,WrongFormat,DuplicateIDException,InvalidFormatException
	{
		IDAREWorkbook WB = null;
		try
		{
			WB = new POIWorkBook(WorkbookFactory.create(new FileInputStream(DataFile)));
		}
		catch(Exception e) 
		{
			WB = new POIWorkBook(new CSVWorkbook(DataFile, "\t"));
		}
	...
	{		
		...
		while(rowIterator.hasNext() && uninitialized)
		{
			...
			while(cellIterator.hasNext())
			{
				...
				if(currentcell.getCellType() == Cell.CELL_TYPE_NUMERIC || currentcell.getCellType() == Cell.CELL_TYPE_FORMULA)
				{
					if(uninitialized)
					{
						isnumeric = true;
		...
		{	
			...
			while(rowIterator.hasNext() && isdiscreet)
			{
				...
				while(cellIterator.hasNext())
				{
					...
					if(current.getCellType() == Cell.CELL_TYPE_BLANK)
					{
						entries.add(null);
					}
	...
	/**
	 * Get the number of columns for this set (this excludes the empty columns)
	 * @return 
	 
	public int getSetColumnCount()
	{
		int setColumns = 0;
